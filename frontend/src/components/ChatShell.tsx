...
import { getApiKey, hasApiKey } from "@/lib/local/settings";
...
const bubbleRole = {
  assistant: {
    ...
    footer: (_content: string, info: { extraInfo?: BubbleExtra }) => {
    ...
    <div className="flex h-screen overflow-hidden bg-gray-50">
      ...
      {sidebarOpen && (
        <>
          ...
          />
          <div className="fixed left-0 top-0 z-40 h-full w-64 shadow-xl md:hidden">
            {sidebar}
          </div>
        </>
      )}
        ...
        <div className="flex-1 overflow-hidden">
          <div className="mx-auto flex h-full w-full max-w-3xl flex-col">
            {messages.length === 0 ? (
              <div className="flex flex-1 flex-col items-center justify-center gap-6 overflow-y-auto px-4 py-8">
                <Welcome
                  ...
                  description="营养健康助手，回答基于本地知识库并附参考来源。"
                />
                <Prompts
                  title="试试这样问"
                  wrap
            ...
            <div className="px-4 pb-4">
              <Sender
                placeholder={!apiKey ? "请先配置 API Key…" : "输入你的健康问题…"}
                value={inputValue}
      ...
      <SettingsModal
        ...
        onSaved={(key) => setApiKeyState(key)}
      />
